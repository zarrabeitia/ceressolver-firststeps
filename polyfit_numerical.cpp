...
class PolyFitNumerical
          : public ceres::SizedCostFunction<1,3> //1 residual, 1 parameter block with 3 variables.
{
public:
    PolyFitNumerical(double const observation, double time) {
        this->observation = observation;
        this->time = time;
    }

    virtual ~PolyFitNumerical() {}
    virtual bool Evaluate(double const* const* parameters,
        ...
        residuals[0] = a + b*time + c*time*time - observation;
...
void example_polyfit_numerical()
{
    ...
    for(int t = 0; t<100; t++) {
        ceres::CostFunction *costFunction = new ceres::NumericDiffCostFunction<PolyFitNumerical, ceres::CENTRAL, 1, 3>(new PolyFitNumerical(observations[t], t), ceres::TAKE_OWNERSHIP);
