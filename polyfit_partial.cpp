...
class PolyFitPartial
          : public ceres::SizedCostFunction<1,3> //1 residual, 1 parameter block with 3 variables.
{
public:
    PolyFitPartial(double const observation, double time) {
        this->observation = observation;
        this->time = time;
    }

    virtual ~PolyFitPartial() {}
    virtual bool Evaluate(double const* const* parameters,
        ...
        if (jacobians != NULL) {
            ...
            // This is still messy, and can get worse!
...
void example_polyfit_partial()
{
    ...
    for(int t = 0; t<100; t++) {
        problem.AddResidualBlock(new PolyFitPartial(observations[t], t), NULL, params);
    }
